...
export async function geraEvento({ config, clientes }) {
    ...
    try {
        ...
        for (index; index < clientes.length; index++) {
            ...
            error1 = await page.$eval('#perfilAcesso > div.erro > p', item => item.textContent.trim()).catch(e => 'ATENÇÃO:')
            if (error1 != "ATENÇÃO:") {
            ...
            await frame.waitForSelector('#nav > li:nth-child(2) > ul > li:nth-child(9) > a')
            ...
            await frame.click('#divBody > form:nth-child(3) > fieldset > div > div:nth-child(2) > input');
            await page.waitForNetworkIdle()
            const tabelaEventos = await frame.$$('#tabelaEventos tr', element => element).catch(e => 'sem tabela')
            let existePeriodoCadastrado = false
            if (tabelaEventos !== 'sem tabela' || tabelaEventos.length !== 0) {
                for (let y = 2; y <= tabelaEventos.length; y++) {
                    const periodoApuracao = await frame.$eval(`#tabelaEventos tr:nth-child(${y}) td:nth-child(1)`, element => element.textContent.trim())
                    console.log(periodoApuracao)
                    if (periodoApuracao === config.periodoApuracao) {
                        console.log('Periodo ja cadastrado para o CNPJ ' + clientes[index].CNPJ.toString())
                        existePeriodoCadastrado = true
                        const protocolo = await frame.$eval(`#tabelaEventos tr:nth-child(${y}) td:nth-child(7)`, element => element.textContent.trim())
            ...
            await frame.click('#divBody > form:nth-child(4) > fieldset:nth-child(2) > a')
            ...
            await frame.waitForSelector('#divBody > form > fieldset > span:nth-child(5)')
            const protocolo = await frame.$eval('#divBody > form > fieldset > span:nth-child(5)', element => element.textContent.trim())
            const html = await mountHTML('evento.html', [
                `${clientes[index].CNPJ.toString()} - ${clientes[index].RAZAO}`,
                protocolo,
                config.periodoApuracao
